...
export const getTelegramConnection = createServerFn({ method: "GET" })
...
export const createTelegramLink = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    return {
      botLink: `https://t.me/${botUsername}?start=${encodeURIComponent(token)}`,
      expiresAt,
    };
  });

export const updateTelegramPreferences = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: connection, error } = await supabaseAdmin
      ...
      .single();
    if (error) throw error;
    return connection as TelegramConnection;
  });
